...
def training(model, x, y, round, bs=64, ep=1024):
    ...
    for cnt, (x_, y_) in enumerate(ds):
        ...
        # with sw.as_default():
        #     tf.summary.scalar('loss%d' % round, loss.numpy(), step=cnt)
...
def main():
    experiment_num = 10
    gb_num = 3
    data, target, att_dim, res_dim = dataset_numeric_classification('wine', 1)
    ...
    for en in range(experiment_num):
        for train_data, train_target, test_data, test_target in kfold(data, target, 10, 'numeric'):
            ...
            models, preds, one_hot_target = [], [], np.eye(res_dim)[train_target]
            now_target = one_hot_target
            for gn in range(gb_num):
                model = Model(16, att_dim, res_dim)
                training(model, (train_data - O) / D, now_target, gn)
                models.append(model), preds.append(model.predict((train_data - O) / D))
                now_pred = tf.reduce_prod(preds, 0)
                now_pred = now_pred / tf.expand_dims(tf.reduce_sum(now_pred, -1), -1)
                now_target = tf.nn.softmax(one_hot_target - now_pred)

                cc = tf.reduce_mean(tf.keras.losses.categorical_crossentropy(one_hot_target, now_pred))
                print("%d:%f" % (gn, cc.numpy()))

            test_pred = [model.predict((test_data - O) / D) for model in models]
            test_pred = tf.reduce_prod(test_pred, 0)
            test_pred = test_pred / tf.expand_dims(tf.reduce_sum(test_pred, -1), -1)
            test_pred = tf.argmax(test_pred, -1)

            acci.reset_states()
            acci.update_state(test_target, test_pred)
            acc.update_state(test_target, test_pred)
            print("%f" % acci.result().numpy())
        print("acc=%f" % acc.result().numpy())
